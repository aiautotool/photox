...
export type ResumableFinalizeLedgerOptions = { rootDir: string; now?: () => number };
...
function isRecord(value: unknown): value is ResumableFinalizeRecord {
  ...
  return (row.version === 1 || row.version === 2)
    && typeof row.sessionId === 'string' && typeof row.workspaceId === 'string' && typeof row.deviceId === 'string'
    ...
    && Number.isSafeInteger(row.expectedBytes) && Number(row.expectedBytes) > 0
    && typeof row.key === 'string' && typeof row.sha256 === 'string' && typeof row.committedAtIso === 'string';
}
...
export class ResumableFinalizeLedger {
  ...
  constructor(options: ResumableFinalizeLedgerOptions) { this.rootDir = options.rootDir; this.now = options.now ?? Date.now; }
  private filePath(sessionId: string) { return path.join(this.rootDir, `${identity(sessionId, 'UPLOAD_SESSION_ID_REQUIRED')}.finalize.json`); }
  private async ensureRoot() { await fs.mkdir(this.rootDir, { recursive: true, mode: 0o700 }); }

  async get(sessionId: string, binding?: { workspaceId: string; deviceId: string; actorUserId?: string }): Promise<ResumableFinalizeRecord | null> {
    let parsed: unknown;
    try { parsed = JSON.parse(await fs.readFile(this.filePath(sessionId), 'utf8')); }
    catch (error) { if ((error as NodeJS.ErrnoException).code === 'ENOENT') return null; throw new Error('UPLOAD_FINALIZE_LEDGER_INVALID'); }
    if (!isRecord(parsed) || parsed.sessionId !== sessionId) throw new Error('UPLOAD_FINALIZE_LEDGER_INVALID');
    if (binding && (parsed.workspaceId !== binding.workspaceId || parsed.deviceId !== binding.deviceId || (parsed.actorUserId !== undefined && parsed.actorUserId !== binding.actorUserId))) throw new Error('UPLOAD_FINALIZE_BINDING_MISMATCH');
    return parsed;
  }

  async markCommitted(input: Omit<ResumableFinalizeRecord, 'version' | 'committedAtIso'>): Promise<ResumableFinalizeRecord> {
    await this.ensureRoot();
    const actorUserId = input.actorUserId ? identity(input.actorUserId, 'USER_SCOPE_REQUIRED') : undefined;
    const record: ResumableFinalizeRecord = {
      version: actorUserId ? 2 : 1,
    ...
    if (existing) {
      if (existing.workspaceId !== record.workspaceId || existing.deviceId !== record.deviceId || existing.actorUserId !== record.actorUserId || existing.reservationId !== record.reservationId || existing.expectedBytes !== record.expectedBytes || existing.key !== record.key || existing.sha256 !== record.sha256) throw new Error('UPLOAD_FINALIZE_LEDGER_CONFLICT');
      return existing;
    }
    await atomicWrite(this.filePath(record.sessionId), record);
    return record;
  }
  async remove(sessionId: string) { await fs.rm(this.filePath(sessionId), { force: true }); }
}
