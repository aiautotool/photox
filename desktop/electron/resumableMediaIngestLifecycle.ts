...
export type ResumableQuotaReservation = { reservationId: string };
...
function requiredIdentity(value: string | undefined, code: string) {
...
function binding(principal: ResumableIngestPrincipal) {
  const actorUserId = principal.actorUserId ? requiredIdentity(principal.actorUserId, 'USER_SCOPE_REQUIRED') : undefined;
  return {
...
export function createResumableMediaIngestLifecycle<T>(deps: ResumableIngestLifecycleDependencies<T>) {
  ...
  async function create(principal: ResumableIngestPrincipal, input: Omit<CreateResumableMediaSessionInput, 'workspaceId' | 'deviceId' | 'actorUserId' | 'quotaReservationId'>) {
    ...
    try {
      ...
      if (reservation && deps.quota) await deps.quota.release({ principal: actor, reservationId: reservation.reservationId, expectedBytes: input.expectedBytes, reason: 'create_failed' });
      throw error;
    }
  }

  async function status(principal: ResumableIngestPrincipal, sessionId: string) {
    return deps.store.get(requiredIdentity(sessionId, 'UPLOAD_SESSION_ID_REQUIRED'), binding(principal));
  }
  ...
  async function finalize(principal: ResumableIngestPrincipal, input: { sessionId: string; sha256: string }) {
    ...
    if (complete.session.version === 2 && !complete.session.actorUserId) throw new Error('UPLOAD_SESSION_ACTOR_REQUIRED');
    ...
    if (priorCommit) {
      if (priorCommit.key !== key || priorCommit.sha256 !== authoritativeSha256 || priorCommit.expectedBytes !== complete.session.expectedBytes || priorCommit.reservationId !== reservationId) throw new Error('UPLOAD_FINALIZE_LEDGER_CONFLICT');
      if (reservationId && deps.quota) await deps.quota.commit({ principal: actor, reservationId, expectedBytes: complete.session.expectedBytes, key });
    ...
    const outcome = await coordinator.run({ workspaceId: complete.session.workspaceId, key }, {
      ...
      commit: () => deps.commit({ workspaceId: complete.session.workspaceId, key, session: complete.session, partPath: complete.partPath, sha256: authoritativeSha256 }),
    });
    ...
    if (outcome.status === 'committed' && deps.finalizeLedger) {
      await deps.finalizeLedger.markCommitted({
        ...
        actorUserId: complete.session.actorUserId,
        reservationId,
    ...
    if (reservationId && deps.quota) {
      if (outcome.status === 'duplicate') await deps.quota.release({ principal: actor, reservationId, expectedBytes: complete.session.expectedBytes, reason: 'duplicate' });
      else await deps.quota.commit({ principal: actor, reservationId, expectedBytes: complete.session.expectedBytes, key });
    }
    await deps.finalizeLedger?.remove(sessionId);
    await deps.store.remove(sessionId);
  ...
  async function cleanupExpired() {
    return deps.store.cleanupExpired(async session => {
      ...
      if (!reservationId || !deps.quota) { await deps.finalizeLedger?.remove(session.sessionId); return; }
      const actor = { workspaceId: session.workspaceId, deviceId: session.deviceId, actorUserId: session.actorUserId };
      const priorCommit = await deps.finalizeLedger?.get(session.sessionId, actor);
      if (priorCommit) {
        const key = `${session.deviceId}:${session.assetId}`;
        if (priorCommit.key !== key || priorCommit.expectedBytes !== session.expectedBytes || priorCommit.reservationId !== reservationId) throw new Error('UPLOAD_FINALIZE_LEDGER_CONFLICT');
      ...
      await deps.quota.release({ principal: actor, reservationId, expectedBytes: session.expectedBytes, reason: 'expired' });
    });
  }
