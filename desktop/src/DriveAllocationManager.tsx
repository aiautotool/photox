...
export function DriveAllocationManager(){
  ...
  useEffect(()=>{
    const locate=()=>setTarget(document.querySelector('.accounts-panel'));
    locate();
  ...
  useEffect(()=>{if(target)void refresh()},[target]);
  ...
  return createPortal(<section className="drive-allocation-inline" aria-label="Phân bổ Google Drive">
    <header className="drive-allocation-header"><div><span className="drive-allocation-kicker">GOOGLE DRIVE ALLOCATION</span><h3>Phân bổ dung lượng PhotoX</h3><p>Mỗi tài khoản mặc định dùng tối đa 66,67% quota tổng do Google báo cáo. PhotoX còn tôn trọng dung lượng thực còn lại và safety reserve trước khi ghi file mới.</p></div><button disabled={loading||busy!==null} onClick={()=>void refresh()}>{loading?'Đang tải…':'Làm mới quota'}</button></header>
    <div className="drive-allocation-summary"><div><b>{accounts.length} tài khoản</b><span>Không có giới hạn 10 GB cố định.</span></div><div><b>2/3 mặc định</b><span>Có thể cấu hình riêng cho từng tài khoản.</span></div></div>
