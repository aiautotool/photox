...
const formatBytes=(bytes:number)=>bytes>=1024**3?`${(bytes/1024**3).toFixed(1)} GB`:bytes>=1024**2?`${(bytes/1024**2).toFixed(1)} MB`:bytes>=1024?`${(bytes/1024).toFixed(1)} KB`:`${bytes} B`;
...
export function MigrationPage({bridge}:Props){
  ...
  const percent=job?.totalItems?Math.round(job.completedItems/job.totalItems*100):0;
