...
import { buildBackupProblemViews, backupReplicaStatusLabel, repairBackupProblem } from './backupHealthUi';
import { MigrationPage } from './MigrationPage';
import { DevicesPage } from './DevicesPage';
import './MigrationPage.css';
import './BackupProblems.css';
...
export function App(){
  ...
  const [clearLocalResult,setClearLocalResult]=useState<string>('');

  async function refresh(){
    const b=bridge;if(!b)return;
    const results=await Promise.allSettled([b.getStatus(),b.listLocalMedia(),b.getTunnelStatus(),b.listCloudUploads(),b.listGoogleAccounts(),b.getBackupHealth()] as const);
    const [s,m,t,u,a,h]=results;
    ...
    if(failed?.status==='rejected')console.error('PhotoSync refresh partially failed',failed.reason);
  }
  useEffect(()=>{void refresh();const b=bridge;if(!b)return;const off1=b.onFileReceived(()=>void refresh());const off2=b.onStorageUpdated(()=>void refresh());const off3=b.onTunnelState(t=>setTunnel(t));const timer=setInterval(()=>void refresh(),8000);return()=>{off1();off2();off3();clearInterval(timer)}},[]);
  ...
  const problemViews=useMemo(()=>buildBackupProblemViews(health,uploads,media),[health,uploads,media]);
  ...
  async function addGoogle(){const b=bridge;if(!b)return;try{setStatus(await b.addGoogleAccount());await refresh()}catch(e){setStatus(s=>({...s,state:'error',message:e instanceof Error?e.message:String(e)}))}}
  async function retryCloud(){const b=bridge;if(!b)return;setStatus(await b.retryCloud());await refresh()}
  async function repairProblem(key:string){if(!bridge||repairingKey)return;setRepairingKey(key);try{await repairBackupProblem(bridge,key);await refresh()}catch(e){setStatus(s=>({...s,state:'error',message:e instanceof Error?e.message:String(e)}))}finally{setRepairingKey(null)}}
  ...
  async function clearLocalCache(){const action=bridge?.clearLocalCache;if(!action||clearingLocal)return;if(!window.confirm('Chỉ xóa file local đã có ít nhất 2 bản Google Drive được xác minh. Tiếp tục?'))return;setClearingLocal(true);try{const result=await action();setClearLocalResult(`Đã giải phóng ${formatBytes(result.bytesFreed)} từ ${result.cleared} mục; giữ lại ${result.skipped} mục chưa đủ an toàn.`);await refresh()}catch(e){setClearLocalResult(e instanceof Error?e.message:String(e))}finally{setClearingLocal(false)}}
  ...
  function renderContent(){
    ...
    if(active==='Thiết bị')return <DevicesPage bridge={bridge} pairingCard={pairingCard} connectionReady={tunnel.connected} lastRunAt={status.lastRunAt}/>;
    ...
    if(active==='Vấn đề')return <section className="page-section problems-page"><div className="section-title"><div><h2>Vấn đề cần xử lý</h2><p>Repair từng dòng chỉ xử lý đúng media đã chọn; “Sửa tất cả có thể” mới chạy repair sweep toàn workspace.</p></div><button className="sync-now compact-button" onClick={()=>void retryCloud()}>Sửa tất cả có thể</button></div><div className="problem-summary"><div><b>{health.critical}</b><span>Nguy cấp</span></div><div><b>{health.atRisk}</b><span>Có rủi ro</span></div><div><b>{health.unknown}</b><span>Chưa xác minh</span></div></div>{problemViews.length?<div className="problem-list">{problemViews.map(problem=>{const expanded=expandedProblemKey===problem.key;return <article className={`problem-item problem-${problem.health}`} key={problem.key}><div className="problem-icon">{problem.health==='critical'?'!':'?'}</div><div><b>{problem.filename}</b><small>{problem.verifiedReplicas} / {problem.targetReplicas} replica Drive đã xác minh · {problem.failedReplicas} lỗi · {problem.pendingReplicas} đang chờ</small><small className="problem-reason">{problem.reason}</small>{!problem.localAvailable&&<span className="problem-local-warning">Original không còn trên máy; PhotoX không thể tạo replica mới từ file local.</span>}</div><div className="problem-actions"><button onClick={()=>setExpandedProblemKey(expanded?null:problem.key)}>{expanded?'Ẩn chi tiết':'Chi tiết'}</button><button className="sync-now compact-button" disabled={!problem.canRepair||repairingKey!==null} onClick={()=>void repairProblem(problem.key)}>{repairingKey===problem.key?'Đang sửa…':'Repair'}</button></div>{expanded&&<div className="problem-details"><div className="problem-detail-summary"><span>Local: <b>{problem.localAvailable?'Có original':'Thiếu original'}</b></span><span>Verified: <b>{problem.verifiedReplicas}/{problem.targetReplicas}</b></span><span>Failed: <b>{problem.failedReplicas}</b></span><span>Pending: <b>{problem.pendingReplicas}</b></span></div>{problem.replicas.length?<div className="problem-replicas">{problem.replicas.map((replica,index)=><div className="problem-replica" key={`${replica.accountId||replica.state}:${index}`}><div><b>{replica.accountEmail||replica.accountId||`Replica ${index+1}`}</b><small>{replica.message||'Không có lỗi provider được ghi nhận.'}</small></div><span className={`problem-replica-state ${replica.state==='VERIFIED'||replica.state==='UPLOADED'?'is-good':replica.state==='ERROR'?'is-error':''}`}>{backupReplicaStatusLabel(replica.state)}</span></div>)}</div>:<span className="problem-empty-replica">Chưa có replica Drive nào được tạo cho media này.</span>}</div>}</article>})}</div>:<Empty title="Không có vấn đề" description="Toàn bộ media đang đáp ứng chính sách backup."/>}</section>;
    if(active==='Khôi phục')return <section className="page-section two-column"><div className="panel recovery-panel"><h2>Thư viện ưu tiên cloud</h2><p className="muted">Ảnh vẫn xuất hiện và được đọc từ Google Drive khi file local không còn. Sau khi cài lại, kết nối lại các tài khoản Gmail để dựng lại thư viện.</p><div className="recovery-stat"><b>{health.safe}</b><span>mục có replica đã xác minh</span></div><button className="sync-now" onClick={()=>setActive('Ảnh')}>Mở thư viện</button></div><div className="panel details-panel"><h3>Giải phóng bộ nhớ máy</h3><div className="detail-row"><span>Điều kiện xóa local</span><b>Đủ 2 bản Drive</b></div><div className="detail-row"><span>Sau khi xóa</span><b>Xem trực tiếp từ cloud</b></div><button className="sync-now" disabled={!bridge?.clearLocalCache||clearingLocal} onClick={()=>void clearLocalCache()}>{clearingLocal?'Đang dọn…':'Dọn các bản local đã an toàn'}</button>{clearLocalResult&&<p className="muted">{clearLocalResult}</p>}</div></section>;
    if(active==='Tài khoản lưu trữ')return <section className="page-section accounts-layout"><div className="panel accounts-panel"><div className="panel-head"><div><h3>Tài khoản Google Drive</h3><p>Thêm hoặc ngắt kết nối nơi lưu ảnh</p></div><button className="add-account compact-button" onClick={()=>void addGoogle()}>＋ Thêm tài khoản</button></div>{accounts.length?<div className="drive-account-list">{accounts.map(account=>{const total=account.totalBytes||account.freeBytes;const usedPercent=total?Math.min(100,account.usedBytes/total*100):0;return <article className="drive-account" key={account.id}><div className="google">G</div><div className="drive-account-main"><div className="drive-account-title"><div><b>{account.email}</b><small>{account.status==='ready'?'Đã kết nối':'Cần kết nối lại'}</small></div><button disabled={removingAccount===account.id} className="remove-account" onClick={()=>void removeGoogle(account)}>{removingAccount===account.id?'Đang xóa…':'Ngắt kết nối'}</button></div>{account.status==='ready'?<><div className="progress"><i style={{width:`${usedPercent}%`}}/></div><div className="quota-row"><span>Đã dùng {formatBytes(account.usedBytes)} / {formatBytes(total)}</span><b>Còn trống {formatBytes(account.freeBytes)}</b></div></>:<div className="quota-unavailable">Không đọc được dung lượng. Hãy kết nối lại tài khoản.</div>}</div></article>})}</div>:<Empty title="Chưa có tài khoản" description="Thêm Google Drive để ảnh luôn có một bản sao an toàn."/>}<button className="sync-now" onClick={()=>void retryCloud()}>Sao lưu ngay</button></div><div className="panel details-panel"><h3>Thư viện trên máy</h3><div className="big-stat">{media.length}<small>ảnh và video</small></div><p className="muted">Ảnh gốc được lưu trong thư viện PhotoSync trên máy này.</p><button className="sync-now" onClick={()=>bridge?.openLibrary()}>Mở thư viện</button></div></section>;
    return <section className="page-section settings-list"><div className="panel"><h3>Hiển thị</h3><label className="setting-row"><div><b>Lưới ảnh thu gọn</b><small>Hiển thị nhiều ảnh hơn trên một hàng.</small></div><input type="checkbox" checked={compact} onChange={e=>{setCompact(e.target.checked);localStorage.setItem('photosync.compact',String(e.target.checked))}}/></label></div><div className="panel"><h3>Dữ liệu</h3><div className="setting-row"><div><b>Thư viện trên máy</b><small>{status.libraryPath||'Pictures/PhotoSync'}</small></div><button onClick={()=>bridge?.openLibrary()}>Mở thư mục</button></div><div className="setting-row"><div><b>Làm mới trạng thái</b><small>Cập nhật ảnh, kết nối và tài khoản lưu trữ.</small></div><button onClick={()=>void refresh()}>Làm mới</button></div></div>{bridge?.platform!=='web'&&<div className="panel"><h3>Web</h3><div className="setting-row"><div><b>Liên kết đăng nhập Web một lần</b><small>{webLoginLink?`Đã sao chép. Hết hạn ${new Date(webLoginLink.expiresAt).toLocaleTimeString('vi-VN')}`:'Tạo link ngắn hạn để mở đúng giao diện Desktop trên trình duyệt. Refresh token không đi qua JavaScript.'}</small></div><button onClick={()=>void createWebLoginLink()}>Tạo & sao chép</button></div>{webLoginLink&&<div className="setting-row"><div><b>Link Web</b><small style={{wordBreak:'break-all'}}>{webLoginLink.url}</small></div></div>}</div>}</section>;
  }
