...
type BackupHealthSnapshot = {total:number;safe:number;atRisk:number;critical:number;unknown:number;photos:number;videos:number;totalBytes:number;problems:{key:string;filename:string;health:'at_risk'|'critical'|'unknown';reason:string}[]};
type DesktopBridge = {
  platform:string;
...
export function App(){
  ...
  async function refresh(){const b=window.photoSyncDesktop;if(!b)return;const [s,m,t,u,a,h]=await Promise.all([b.getStatus(),b.listLocalMedia(),b.getTunnelStatus(),b.listCloudUploads(),b.listGoogleAccounts(),b.getBackupHealth()]);setStatus(s);setMedia(m);setTunnel(t);setUploads(u);setAccounts(a);setHealth(h)}
  ...
  function renderContent(){
    ...
    if(active==='Vấn đề')return <section className="page-section problems-page"><div className="section-title"><div><h2>Vấn đề cần xử lý</h2><p>Trạng thái được tính từ các bản sao đã xác minh.</p></div><button className="sync-now compact-button" onClick={()=>void retryCloud()}>Thử sửa lại</button></div><div className="problem-summary"><div><b>{health.critical}</b><span>Nguy cấp</span></div><div><b>{health.atRisk}</b><span>Có rủi ro</span></div><div><b>{health.unknown}</b><span>Chưa xác minh</span></div></div>{health.problems.length?<div className="problem-list">{health.problems.map(problem=><article className={`problem-item problem-${problem.health}`} key={problem.key}><div className="problem-icon">{problem.health==='critical'?'!':'?'}</div><div><b>{problem.filename}</b><small>{problem.health==='critical'?'Chưa đủ bản sao original an toàn.':problem.health==='at_risk'?'Có bản sao nhưng chưa đạt policy.':'Cần kiểm tra lại provider.'}</small></div><button onClick={()=>void retryCloud()}>Thử lại</button></article>)}</div>:<Empty title="Không có vấn đề" description="Toàn bộ media đang đáp ứng chính sách backup."/>}</section>;
    if(active==='Khôi phục')return <section className="page-section two-column"><div className="panel recovery-panel"><h2>Trung tâm khôi phục</h2><p className="muted">Chọn media từ thư viện để xem các nguồn original có thể dùng khi máy này gặp sự cố.</p><div className="recovery-stat"><b>{health.safe}</b><span>mục có replica đã xác minh</span></div><button className="sync-now" onClick={()=>setActive('Ảnh')}>Mở thư viện để chọn media</button></div><div className="panel details-panel"><h3>Nguyên tắc an toàn</h3><div className="detail-row"><span>Original tối thiểu</span><b>2 bản</b></div><div className="detail-row"><span>YouTube</span><b>Chỉ bản xem được</b></div><div className="detail-row"><span>Xóa file gốc</span><b>Không tự động</b></div></div></section>;
