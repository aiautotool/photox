...
Desktop has `driveAccountPolicyStore.ts` as the persistence boundary for these per-account settings. Policy writes are workspace-scoped, atomic, preserve the OAuth credential payload server-side, and legacy unscoped files are adopted only into the configured legacy workspace. Restart and cross-tenant regression coverage is required. Renderer/Web payloads must never reuse the persisted credential record directly because it contains token material.

`driveRuntimeAllocation.ts` is now the integration adapter between persisted Drive policy and runtime account selection. Given a saved workspace-owned account, provider-authoritative `{limit, usage}` counters and PhotoX `appUsedBytes`, it must produce the `StorageAccount` consumed by `chooseAccount()` with the persisted `maxUsageRatio` and `safetyReserveBytes` attached. It also produces a renderer-safe allocation snapshot through `storageAllocationSnapshot()`; that snapshot may expose capacity/policy numbers but must not contain OAuth tokens, workspace binding or provider credentials. Malformed provider counters must fail closed to zero writable capacity.

The remaining main-process wiring is to replace direct `StorageAccount` construction inside `runtimeDriveAccounts()` with `driveRuntimeAllocation()`, include the safe snapshot in `DriveAccountInfo`, and expose owner/admin mutation through Electron IPC and authenticated Web routes with tenant binding, browser CSRF and audit before the shared UI editor is enabled.
