...
function formatDuration(seconds=0){const v=Math.max(0,Math.round(seconds));return `${Math.floor(v/60)}:${String(v%60).padStart(2,'0')}`;}
...
export default function MobileHome(){
  ...
  const [addTargetAlbumId,setAddTargetAlbumId]=useState<string|null>(null);
  ...
  const addTargetAlbum=library.albums.find(a=>a.id===addTargetAlbumId)||null;
  ...
  async function autoSync(currentTarget:PairedDesktop,assets:MediaAsset[],retryFailed=false,override?:BackupSettings){
    ...
    const pending=assets.filter(a=>(a.mediaType==='video'?settings.backupVideos:settings.backupPhotos)&&!knownSynced.has(a.id)&&(retryFailed||!knownFailed[a.id]));setSyncedIds(knownSynced);setFailedAssets(knownFailed);
    ...
    try{await pingLaptop(currentTarget,controller.signal);setConnected(true);setSyncPhase('syncing');setProgress({total:pending.length,completed:0,skipped:0,failed:0});await syncAssetsToLaptop(currentTarget,pending,async v=>{setProgress(v);setSyncedIds(await loadSyncedAssetIds());setFailedAssets(await loadFailedAssets());},controller.signal);setSyncedIds(await loadSyncedAssetIds());setFailedAssets(await loadFailedAssets());await refreshCloudLibrary(currentTarget,assets);}catch{if(!controller.signal.aborted)setConnected(false);}finally{syncingRef.current=false;syncControllerRef.current=null;setSyncPhase(controller.signal.aborted?'paused':'idle');}
  }
  ...
  function cancelSelection(){const returnAlbum=addTargetAlbumId;setSelected(new Set());setAddTargetAlbumId(null);if(returnAlbum){setTab('collections');setCustomAlbumId(returnAlbum);}}

  async function openEditor(asset:DisplayAsset){if(asset.mediaType==='video'){setMessage('Chỉnh sửa video chưa hỗ trợ.');return;}try{const source=await prepareAssetForEditing(asset);router.push({pathname:'/editor',params:{id:asset.id,uri:source,filename:asset.filename,width:asset.width?String(asset.width):undefined,height:asset.height?String(asset.height):undefined,mimeType:asset.filename.toLowerCase().endsWith('.png')?'image/png':'image/jpeg'}});}catch(e){setMessage(`Không mở được editor: ${e instanceof Error?e.message:String(e)}`);}}
  async function shareAsset(asset:DisplayAsset){try{let uri=asset.uri;if(asset.cloudOnly){const downloaded=await downloadCloudAsset(asset);uri=downloaded.uri;}if(!(await Sharing.isAvailableAsync()))throw new Error('SHARING_UNAVAILABLE');await Sharing.shareAsync(uri,{dialogTitle:`Chia sẻ ${asset.filename}`});}catch(e){Alert.alert('Không thể chia sẻ',e instanceof Error?e.message:String(e));}}
  ...
  async function emptyTrash(){Alert.alert('Dọn sạch Thùng rác?','Các file local sẽ bị xóa vĩnh viễn.',[{text:'Hủy',style:'cancel'},{text:'Dọn sạch',style:'destructive',onPress:()=>void (async()=>{const byId=new Map(devicePhotos.map(x=>[x.id,x]));const trashIds=library.trash.map(x=>x.mediaId);const result=await mobileLibraryController.emptyTrash(id=>byId.get(id) as unknown as MediaLibrary.Asset|undefined);const removed=new Set(trashIds.filter(id=>byId.has(id)&&!result.failed.includes(id)));setDevicePhotos(old=>old.filter(x=>!removed.has(x.id)));setPhotos(old=>old.filter(x=>!removed.has(x.id)));if(result.failed.length)Alert.alert('Một số mục chưa xóa được',`${result.failed.length} mục thất bại.`);})()}]);}
  ...
  async function batchShare(){const first=photos.find(x=>selected.has(x.id));if(first)await shareAsset(first);if(selected.size>1)Alert.alert('Chia sẻ nhiều mục','Native share hiện nhận từng file. PhotoX không giả lập batch share; các mục còn lại vẫn được giữ nguyên.');}
  async function addSelectedToTarget(){if(!addTargetAlbumId||!selected.size)return;const id=addTargetAlbumId;await mobileLibraryController.addToAlbum(id,Array.from(selected));setSelected(new Set());setAddTargetAlbumId(null);setTab('collections');setCustomAlbumId(id);}
  async function removeSelectedFromCurrentAlbum(){if(!currentCustomAlbum||!selected.size)return;await mobileLibraryController.removeFromAlbum(currentCustomAlbum.id,Array.from(selected));setSelected(new Set());}
  function beginAddToAlbum(albumId:string){setAddTargetAlbumId(albumId);setCustomAlbumId(null);setCollection(null);setSmartAlbum(null);setSelected(new Set());setTab('photos');}
  ...
  function renderTile(asset:DisplayAsset,index:number,trashMode=false){const chosen=selected.has(asset.id);return <Pressable key={asset.id} style={[s.tile,chosen&&s.selectedTile]} onPress={()=>addTargetAlbumId||selected.size?toggleSelect(asset.id):setViewer(asset)} onLongPress={()=>toggleSelect(asset.id)}><Image source={imageSource(asset)} style={s.photo} contentFit="cover" transition={120}/>{asset.mediaType==='video'&&<View style={s.videoBadge}><Text style={s.videoText}>▶ {asset.duration?formatDuration(asset.duration):''}</Text></View>}{(asset.cloudOnly||syncedIds.has(asset.id))&&index<4&&<View style={s.cloudBadge}><Text style={s.cloudText}>✓</Text></View>}{trashMode&&<View style={s.trashBadge}><Text style={s.trashBadgeText}>Trash</Text></View>}{chosen&&<View style={s.check}><Text style={s.checkText}>✓</Text></View>}</Pressable>;}
  function renderGrid(items:DisplayAsset[],trashMode=false){return <View style={s.grid}>{items.map((asset,index)=>renderTile(asset,index,trashMode))}</View>;}
  ...
  const selectionActive=selected.size>0||Boolean(addTargetAlbumId);
  ...
  return <SafeAreaView style={s.root}><StatusBar barStyle="dark-content" backgroundColor="#fff"/>
    ...
    {tab==='collections'&&customAlbumId&&currentCustomAlbum&&<SubPage title={currentCustomAlbum.name} onBack={()=>{setSelected(new Set());setCustomAlbumId(null);}} right={<Pressable onPress={()=>beginAddToAlbum(currentCustomAlbum.id)}><Text style={s.link}>＋ Thêm ảnh</Text></Pressable>}>{customAlbumItems.length?renderGrid(customAlbumItems):<Empty title="Album trống" body="Nhấn Thêm ảnh để chọn ảnh từ Thư viện."/>}<View style={s.albumManage}><Pressable onPress={()=>{setAlbumName(currentCustomAlbum.name);setSheet('rename-album');}}><Text style={s.link}>Đổi tên</Text></Pressable><Pressable onPress={()=>Alert.alert('Xóa album?',currentCustomAlbum.name,[{text:'Hủy',style:'cancel'},{text:'Xóa',style:'destructive',onPress:()=>void mobileLibraryController.deleteAlbum(currentCustomAlbum.id).then(()=>{setSelected(new Set());setCustomAlbumId(null);})}])}><Text style={s.danger}>Xóa album</Text></Pressable></View></SubPage>}
    ...
    <Modal visible={sheet!==null} transparent animationType="slide" onRequestClose={()=>setSheet(null)}><Pressable style={s.modalShade} onPress={()=>setSheet(null)}/><View style={s.sheet}><View style={s.handle}/>
      ...
      {sheet==='settings'&&<><Text style={s.sheetTitle}>Cấu hình tải lên</Text><Setting label="Tự động sao lưu" value={backupEnabled} onChange={setBackupEnabled}/><Setting label="Sao lưu ảnh" value={backupPhotos} onChange={setBackupPhotos}/><Setting label="Sao lưu video" value={backupVideos} onChange={setBackupVideos}/><Text style={s.sheetBody}>Cấu hình được lưu trên thiết bị và áp dụng cho cả sao lưu tự động lẫn hàng đợi tiếp theo.</Text></>}
      {sheet==='create-album'&&<><Text style={s.sheetTitle}>Tạo album</Text><TextInput autoFocus value={albumName} onChangeText={setAlbumName} placeholder="Tên album" style={s.albumInput}/><Pressable disabled={!albumName.trim()} style={[s.primary,!albumName.trim()&&s.disabledButton]} onPress={()=>void mobileLibraryController.createAlbum(albumName,Array.from(selected)).then(()=>{setAlbumName('');setSelected(new Set());setSheet(null);setTab('collections');})}><Text style={s.primaryText}>Tạo album</Text></Pressable></>}
      {sheet==='rename-album'&&currentCustomAlbum&&<><Text style={s.sheetTitle}>Đổi tên album</Text><TextInput autoFocus value={albumName} onChangeText={setAlbumName} placeholder="Tên album" style={s.albumInput}/><Pressable disabled={!albumName.trim()} style={[s.primary,!albumName.trim()&&s.disabledButton]} onPress={()=>void mobileLibraryController.renameAlbum(currentCustomAlbum.id,albumName).then(()=>{setAlbumName('');setSheet(null);})}><Text style={s.primaryText}>Lưu tên</Text></Pressable></>}
    ...
    <Modal visible={viewer!==null} animationType="fade" onRequestClose={()=>setViewer(null)}><SafeAreaView style={s.viewer}>{viewer&&<><View style={s.viewerHeader}><Pressable onPress={()=>setViewer(null)}><Icon name="back" color="#fff"/></Pressable><View><Text style={s.viewerTitle}>{viewer.filename}</Text><Text style={s.viewerSub}>{new Date(viewer.creationTime).toLocaleDateString('vi-VN')}</Text></View><Pressable onPress={()=>setViewerInfo(v=>!v)}><Icon name="more" color="#fff"/></Pressable></View><SwipeMediaStage assets={viewerSequence.length?viewerSequence:[viewer]} current={viewer} onChange={asset=>{setViewer(asset);setViewerInfo(false);}}/>{viewerInfo?<ScrollView style={s.details}><Text style={s.sheetTitle}>Chi tiết</Text><Text style={s.detail}>{viewer.width&&viewer.height?`${viewer.width} × ${viewer.height}`:''} {formatBytes(viewerMetadata?.fileSize||viewer.fileSize)}</Text>{viewer.duration?<Text style={s.detail}>Thời lượng: {formatDuration(viewer.duration)}</Text>:null}<Text style={s.detail}>{viewer.cloudOnly?'Cloud':'Local'} • {syncedIds.has(viewer.id)||viewer.cloudOnly?'Đã sao lưu':'Chưa sao lưu'}</Text>{currentCustomAlbum&&currentCustomAlbum.mediaIds.includes(viewer.id)&&<><Pressable style={s.textButton} onPress={()=>void mobileLibraryController.setAlbumCover(currentCustomAlbum.id,viewer.id)}><Text style={s.link}>{currentCustomAlbum.coverMediaId===viewer.id?'✓ Ảnh bìa hiện tại':'Đặt làm ảnh bìa album'}</Text></Pressable><Pressable style={s.textButton} onPress={()=>void mobileLibraryController.removeFromAlbum(currentCustomAlbum.id,[viewer.id]).then(()=>setViewer(null))}><Text style={s.danger}>Xóa khỏi album</Text></Pressable></>}{trashed.has(viewer.id)?<><Pressable style={s.primary} onPress={()=>void restoreAsset(viewer)}><Text style={s.primaryText}>Khôi phục</Text></Pressable><Pressable style={s.textButton} onPress={()=>void deletePermanently(viewer)}><Text style={s.danger}>Xóa vĩnh viễn</Text></Pressable></>:<><Pressable style={s.textButton} onPress={()=>void mobileLibraryController.archive(viewer.id,!archived.has(viewer.id))}><Text style={s.link}>{archived.has(viewer.id)?'Bỏ lưu trữ':'Lưu trữ'}</Text></Pressable>{viewer.cloudOnly&&<Pressable style={s.textButton} onPress={()=>void deletePermanently(viewer)}><Text style={s.danger}>Xóa khỏi cloud</Text></Pressable>}</>}</ScrollView>:<View style={s.viewerToolbar}><Action dark icon="share" label="Chia sẻ" onPress={()=>void shareAsset(viewer)}/>{viewer.mediaType==='photo'&&<Action dark icon="edit" label="Chỉnh sửa" onPress={()=>void openEditor(viewer)}/>}<Action dark icon={favorites.has(viewer.id)?'favoriteFill':'favorite'} label="Yêu thích" onPress={()=>void mobileLibraryController.favorite(viewer.id)}/>{viewer.cloudOnly&&<Action dark icon="download" label={downloading===viewer.id?'Đang tải':'Tải xuống'} onPress={()=>void downloadOne(viewer)}/>}<Action dark icon="trash" label="Xóa" onPress={()=>void trashAsset(viewer)}/></View>}</>}</SafeAreaView></Modal>
...
function albumItems(album:MobileAlbum|null,photos:DisplayAsset[]){if(!album)return[];const byId=new Map(photos.map(p=>[p.id,p]));const items=album.mediaIds.map(id=>byId.get(id)).filter(Boolean) as DisplayAsset[];if(!album.coverMediaId)return items;const cover=byId.get(album.coverMediaId);return cover?[cover,...items.filter(x=>x.id!==cover.id)]:items;}
...
const s=StyleSheet.create({
  root:{flex:1,backgroundColor:'#fff'},flex:{flex:1},header:{height:58,paddingHorizontal:16,flexDirection:'row',alignItems:'center',justifyContent:'space-between'},title:{fontSize:17,fontWeight:'700',color:'#202124',maxWidth:'78%',textAlign:'center'},brand:{fontSize:22,fontWeight:'800',color:'#202124'},headerActions:{flexDirection:'row',alignItems:'center',gap:16},avatar:{width:32,height:32,borderRadius:16,backgroundColor:'#795548',alignItems:'center',justifyContent:'center'},avatarText:{color:'#fff',fontWeight:'700'},scrollBottom:{paddingBottom:110},
